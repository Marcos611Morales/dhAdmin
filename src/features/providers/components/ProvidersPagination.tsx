...
/**
 * Controles de paginación para la tabla de providers.
 *
 * Mismo patrón que UsersPagination:
 * - "X total providers" a la izquierda
 * - "Page X of Y" + botones prev/next a la derecha
 * - Botones se deshabilitan en primera/última página
 */
export function ProvidersPagination({ page, totalPages, total, onPageChange }: ProvidersPaginationProps) {
  if (totalPages <= 0) return null

  return (
    <div className="flex items-center justify-between">
      <p className="text-sm text-gray-500">
        <span className="font-medium text-navy">{total.toLocaleString('en-US')}</span> total providers
      </p>
